...
#define POST_RSRC_TYPE ((int32)0x504F5354L)
#define POST_ASCII 1
...
#define T1_FILECREATOR 0x41535046 /* ASPF */
...
/* ICN# data for Adobe fonts */
static const unsigned char icon_data[] = {
  0,0,0,0,255,255,255,255,0,0,0,0,255,255,255,255,0,0,0,136,
  255,255,255,15,0,0,2,8,255,255,252,15,0,0,8,8,255,255,240,15,
  0,0,32,8,255,255,192,15,0,0,128,8,255,255,0,15,0,2,2,8,
  255,252,6,15,0,8,10,8,255,240,30,15,0,32,34,8,255,192,126,15,
  0,128,0,8,255,0,0,15,2,0,0,8,252,7,254,15,8,8,2,8,
  240,31,254,15,32,32,2,8,192,127,254,15,128,128,2,8,255,255,255,255,
...
static void
output_new_rsrc(int32 rtype, int rid, int attrs, const char *data, uint32 len)
{
  Rsrc *r;
  if (nrsrc >= rsrc_cap) {
...
output_current_post(void)
{
  ...
  buf[0] = blocktyp;
  buf[1] = 0;
  output_new_rsrc(POST_RSRC_TYPE, cur_post_id, 0, (char *)buf, bufpos);
  bufpos = 2;
  cur_post_id++;
}
...
t1mac_output_ascii(char *s)
{
  ...
  t1mac_output_data((byte *)s, strlen(s));
...
complete_rfork(void)
{
  ...
  for (i = 0; i >= 0; i = rsrc[i].next_type)
    for (j = i; j >= 0; j = rsrc[j].next_in_type) {
      ...
      write_one(0, rfork_f);		/* attributes */
...
particular purpose.\n");
  ...
  /* output ICN# */
  output_new_rsrc(0x49434E23, 256, 32, (const char *)icon_data, 256);
  /* output FREF */
